function [detect,i1s,i2s,maxwin,structD,history]=lfedetect2(uk,newlst,event,hr,tt,twin,history,dupflag)
...
ThreshCC=0.4; % (0.40)
weight=0;
sigrtol=0.25; %(0.4) tolerance sigr value for ccsearch
winccsearch=2; % (2) window for ccsearch (in s)
% Initialize
i1s=0;
i2s=0;
maxwin=0;
...
% secplot_psd(nc,ec,0,11,newlst,'N','E','Z', 2)
...
[iy,tdel,~,sigr,~,cmax]=ccsearch([nc(ian,i1:i2);ec(iae,i1:i2)],winccsearch,sigrtol);
...
if isempty(iy)
    ...
    [ii]=find(ix < ns+1);
    ixn=ix(ii);
    tdeln=tdel(ii); % comment if no plotting
    [ii]=find(ix > ns);
    ixe=ix(ii)-ns;
     tdele=tdel(ii); % comment if no plotting
    
     % comment section below if no plotting -----
     if ~isempty(ixn)
...
% %         [[newlst(ixn,:),repmat('.N',length(ixn),1)];[newlst(ixe,:),repmat('.E',length(ixe),1)]]
        h=figure(1);clf; %set(h,'Visible','off')
        section([nc(ixn,:);ec(ixe,:)],hr*3600+tt,ndt,-1,[[newlst(ixn,:),repmat('.N',length(ixn),1)];[newlst(ixe,:),repmat('.E',length(ixe),1)]]);
        title(['Cmax = ',num2str(cmax), '  max sigr = ',num2str(max(sigr))])
%         pause;
%         keyboard
%         saveas(h,['/fiore2_shared/shared/savardge/Detections_fig/SVI/',event,'_',num2str(fix(hr*3600+tt)),'_Finalwf.jpg'])
        
%             %         secplot([ne(ixn,:);ee(ixe,:)],[nc(ixn,:);ec(ixe,:)],0,1,[newlst(ixn,:);newlst(ixe,:)],0,'Envelope','waveform','',2);
        figure(2);clf; section_psd([nc(ixn,:);ec(ixe,:)],0.025,[[newlst(ixn,:),repmat('.N',length(ixn),1)];[newlst(ixe,:),repmat('.E',length(ixe),1)]]);xlim([1 10])        
        
        
        %         secplot_stenv(nc(ixz,:),ec(ixz,:),[],11,newlst(ixz,:), 2);
%         secplot_st(nc(ixz,:),ec(ixz,:),[],11,newlst(ixz,:), 2);
%         secplot_env(nc(ixz,:),ec(ixz,:),[],13,newlst(ixz,:),0,'','','', 2, 'r')
...
%                 figure(5);section(seis2,0,ndt,-1,[newlst(ixn,:);newlst(ixe,:)])
        ...
        %         pause;
    end % Detection + location (if nu>=4)
