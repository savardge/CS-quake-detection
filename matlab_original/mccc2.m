function [tdel,rmean,sigr,r,tcc,tdelw,resw,sigrw] = mccc2(seis,twin,weight)
...
for is=1:ns-1
    ...
    for js=is+1:ns
        ...
        itw=min(nt/2,round(twin(is,js)./(2*ndt)));
        mask=zeros(1,nt);
        mask(1:itw+1)=1.0;
...
%         subplot(3,1,2); plot(seis(js,:))
%         subplot(3,1,3); plot(fftshift(ccfu)./(sigt(is).*sigt(js)))
%         pause
